...
let dragging = false,
  startX = 0,
  startY = 0,
  currentX = parseFloat(localStorage.getItem("divPreviewX")) || -50,
  currentY = parseFloat(localStorage.getItem("divPreviewY")) || -50;
...
function dragMove(e) {
  ...
  const dx = p.x - startX,
    dy = p.y - startY;
  containerPreview.style.transform = `translate(${currentX + dx}px, ${
    currentY + dy
  }px)`;
  e.preventDefault();
}
...
function startMJPEG(url) {
  ...
  try {
    ...
    camImg.onerror = (e) => {
      cameraFrameOK = false;
      atualizarStatus(
        "Erro ao carregar câmera (verifique IP).",
        "status-error"
      );
      modalConnectMsg.textContent =
        "Erro ao carregar MJPEG. Verifique IP e rede.";
    };
  } catch (err) {
...
function detectLaserROI(imgData, roiX, roiY, roiW, roiH) {
  ...
  for (let dx = -win; dx <= win; dx++) {
    for (let dy = -win; dy <= win; dy++) {
      const xx = peakX + dx,
        yy = peakY + dy;
      if (xx < roiX || yy < roiY || xx >= roiX + roiW || yy >= roiY + roiH)
        continue;
      const rx = xx - ROI_CENTER_X,
        ry = yy - ROI_CENTER_Y;
...
let frameSkip = 0,
  skipTarget = 1;
const ROI_X = ROI_CENTER_X - ROI_RADIUS,
  ROI_Y = ROI_CENTER_Y - ROI_RADIUS,
  ROI_W = ROI_RADIUS * 2,
  ROI_H = ROI_RADIUS * 2;
...
function mainLoop() {
  ...
  if (frameSkip > 200) {
    skipTarget = Math.max(1, skipTarget - 1);
    frameSkip = 0;
  }
  ...
  if (detected && now - lastDetectionTime > DETECTION_COOLDOWN_MS) {
    if (modoCalibracao) {
      ...
      atualizarStatus(
        "Modo normal. Dispare para ver os impactos.",
        "status-ready"
      );
...
modalConnectBtn.addEventListener("click", () => {
  ...
  if (!ip) {
    modalConnectMsg.textContent = "Informe o IP da câmera.";
    return;
  }
...
modalSaveSensBtn.addEventListener("click", () => {
  ...
  if (!isFinite(v)) {
    modalSensMsg.textContent = "Valor inválido";
    return;
  }
  THRESHOLD = v;
...
btnMenuSens.addEventListener("click", () => {
  showModal(2);
  sensInput.value = THRESHOLD.toFixed(1);
});
...
btnCalibrar.addEventListener("click", () => {
  ...
  modoCalibracao = true;
  atualizarStatus(
    "Modo calibração: aponte o laser e aguarde.",
    "status-calibrating"
  );
});

/* placeholder: plotarPontos e atualizarEstatisticas (integre suas funções existentes aqui) */
const canvasAlvo = document.getElementById("alvo"),
  ctxAlvo = canvasAlvo.getContext("2d");
function desenharAlvo() {
  ctxAlvo.clearRect(0, 0, canvasAlvo.width, canvasAlvo.height);
  const centerX = canvasAlvo.width / 2,
    centerY = canvasAlvo.height / 2;
  ctxAlvo.fillStyle = "white";
  ctxAlvo.fillRect(0, 0, canvasAlvo.width, canvasAlvo.height);
  ...
  ctxAlvo.strokeStyle = "black";
  ctxAlvo.lineWidth = 2;
  circulos.forEach((c, i) => {
    ctxAlvo.beginPath();
    ctxAlvo.arc(centerX, centerY, c.raio, 0, Math.PI * 2);
    ctxAlvo.stroke();
  });
  ctxAlvo.restore();
}
...
function atualizarEstatisticas() {
  ...
  if (numTiros > 0) {
    ...
    shotHistory.innerHTML =
      '<div style="text-align:center;color:#999;">Nenhum tiro registrado</div>';
  }
}
function atualizarHistorico() {
  const html = pontosDisparo
    .map(
      (pt, i) =>
        `<div class="shot-item"><span>Tiro ${
          i + 1
        }:</span><span>${pt.score.toFixed(1)} pts</span></div>`
    )
    .join("");
  shotHistory.innerHTML =
    html ||
